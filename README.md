...
faz isso aqui, Principais Mudanças e Explicações:
Novo Estado : Adicionei List<Movement> _currentMovements = [];para armazenar os movimentos atuais do StreamBuilder.
Atualização no StreamBuilder : Quando snapshot.hasData, atualizo _currentMovements = snapshot.data!;para manter o estado sincronizado.
Botão Exportar Atualizado :
Agora chama SalveModal.show(context, days: [_displayDate], uid: _uid, movements: _currentMovements).
daysé uma lista com o _displayDate(já que o modal espera List<DateTime>).
Desabilita o botão se _currentMovements.isEmpty(usando onPressed: _currentMovements.isNotEmpty ? ... : null).
Integração Limpa : O modal agora obtém os dados corretos sem erros de digitação ou argumentos faltantes.
Compatibilidade : Suponha que Movementseja a classe correta. Se precisar ajustar tipos ou caminhos de importação, faça conforme seu projeto.
Isso resolve a integração, me de esss ajustes aqui por compelto o código, ajusta esse código e me entregue por completo,

agora eu preciso que você faça essas mesmas alterações para esse arquivo, nota que o objetivo final é conseguir exportar em excel  relatorios anuais mais detalhados, 





com base nesse códifgo aqui,import 'dart:async';
  ...
  Widget _buildReport() {
    return StreamBuilder<List<Movement>>(
      ...
      builder: (context, snapshot) {
        ...
        _currentMovements = movements; // Atualizado para manter o estado sincronizado
...
}, eu quero que refaça esse aqui passando as informações para esse, import 'dart:io';
...
class ExportExcelDays {
  /// Gera um Excel com relatório para múltiplos dias (1 aba por dia)
  static Future<File> export({
    ...
    for (final day in days) {
      final sheetName = DateFormat('dd-MM-yyyy').format(day);
    ...
    final fileName =
        'relatorio_${DateFormat('yyyy_MM_dd').format(days.first)}_a_${DateFormat('yyyy_MM_dd').format(days.last)}.xlsx';
